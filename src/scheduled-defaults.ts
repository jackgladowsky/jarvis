...
const nightlyMemoryReviewPrompt = [
  ...
  "",
  "Purpose: review the previous America/New_York calendar day's JARVIS session logs/summaries and conservatively update persistent memory markdown files where appropriate.",
  "",
  "Procedure:",
  ...
  "2. Determine the previous local date in America/New_York. Review `~/.jarvis/data/notes/recent.md` for archived sessions touching that date. Also read `~/.jarvis/data/sessions/active.json` and consider non-archive `~/.jarvis/data/sessions/*.jsonl` active session logs whose session id, active timestamps, or message timestamps overlap that date; these may not have been rotated into `recent.md` yet.",
  ...
  "- First line must be exactly `NOTIFY: no` when the run completed normally and Jack does not need to see it.",
  "- First line must be exactly `NOTIFY: yes` only when Jack should be alerted about a real problem, ambiguity needing a decision, or notable memory change.",
